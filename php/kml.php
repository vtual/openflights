...
$sql = "SELECT DISTINCT x,y,elevation,iata,icao,name,city,country,count(name) AS visits FROM flights AS f, airports AS a WHERE (f.src_apid=a.apid OR f.dst_apid=a.apid) AND f.uid=" . $uid . getFilterString($db, $_GET) . " GROUP BY name ORDER BY visits DESC";
$result = $db->query($sql);
$first = true;
while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
  $count = $row["visits"];
